...
# question = "llm agent memory"  # {'datasource': 'vectorstore'}
question = "What is prompt?"  # {'datasource': 'web_search'}
docs = retriever.get_relevant_documents(question)
print(f"doc length: {len(docs)}")
print(f"doc >> \n{docs}")  # docs[0].metadata, docs[0].page_content,
...
def grade_generation_v_documents_and_question(state):
    ...
    hallucinationScore = "not supported"
    if grade == "yes":
        ...
        if grade == "yes":
            ...
            hallucinationScore="useful"
        else:
            print("---DECISION: GENERATION DOES NOT ADDRESS QUESTION---")
            hallucinationScore="not useful"
    else:
        pprint("---DECISION: GENERATION IS NOT GROUNDED IN DOCUMENTS, RE-TRY---")

    return {"documents": documents, "question": question, "generation": generation, "hallucinationScore": hallucinationScore}
...
workflow.add_node("grade_hallucination", grade_generation_v_documents_and_question)  # generate
